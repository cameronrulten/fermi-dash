...
def make_template(name: str, title: str, header, tabs):
    ...
    t = pn.template.FastListTemplate(
        title=title, main=[header, tabs], theme_toggle=False
    )  # theme=DarkTheme
...
def build_dashboard(opts: BuildOptions) -> Path:
    ...
    # pn.extension(design="bootstrap", theme="dark") # forces dark unless a template overrides it
    ...
    for name in names:
        ...
        for d, p in pairs:
            ...
            lc_cards.append(
                pn.Card(pane, title=f"Lightcurve — {d:g} days", collapsible=False)
            )
    ...
    console.print(
        f"[dim]Panel {pn.__version__} | Template={type(template).__name__} | theme param type={getattr(template, 'theme', None)}[/dim]"
    )
